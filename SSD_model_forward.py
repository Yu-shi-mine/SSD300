...
from cv2 import CAP_PVAPI_FSTRIGMODE_FIXEDRATE
...
def make_vgg():
    ...
    # vggモジュールで使用する畳み込みやマックスプーリングのチャネル数
    ...
    for v in cfg:
        if v == 'M':
            ...
            layers += [nn.MaxPool2d(kernel_size=2, stride=2, ceil_mode=True)]   # 切り上げモード
        else:
...
def make_loc_conf(num_classes=21, bbox_aspect_num=[4, 6, 6, 6, 4, 4]):
    ...
    # extrasの(source3)に対する畳み込み層
    loc_layers += [nn.Conv2d(512, bbox_aspect_num[2]*4, kernel_size=3, padding=1)]
    conf_layers += [nn.Conv2d(512, bbox_aspect_num[2]*num_classes, kernel_size=3, padding=1)]

    # extrasの(source4)に対する畳み込み層
    loc_layers += [nn.Conv2d(256, bbox_aspect_num[3]*4, kernel_size=3, padding=1)]
    conf_layers += [nn.Conv2d(256, bbox_aspect_num[3]*num_classes, kernel_size=3, padding=1)]

    # extrasの(source5)に対する畳み込み層
    loc_layers += [nn.Conv2d(256, bbox_aspect_num[4]*4, kernel_size=3, padding=1)]
    conf_layers += [nn.Conv2d(256, bbox_aspect_num[4]*num_classes, kernel_size=3, padding=1)]

    # extrasの(source6)に対する畳み込み層
...
class L2Norm(nn.Module):
    ...
    def __init__(self, input_channels=512, scale=20):
        super(L2Norm, self).__init__()  # 親クラスのコンストラクタ
    ...
    def reset_parameters(self):
        init.constant_(self.weight, self.scale) # weightの値がすべて20になる
    
    def forward(self, x):
        # 各チャネルにおける38×38個の特徴量のチャネル方向の2乗和平方根normを計算
        # xをnormで割って正規化
        norm = x.pow(2).sum(dim=1, keepdim=True).sqrt()+self.eps
        x = torch.div(x, norm)

        # 係数weightをかける。係数はチャネルごとに1個割付⇒512個の係数
        # self.weightのテンソルサイズはtorch.size([512])なので
        # torch.Size([batch_size, 512, 38, 38])まで変形
...
class DBox(object):
    def __init__(self, cfg):
        ...
        self.feature_maps = cfg['feature_maps'] # [38, 19, ...]の各sourceの特徴量マップのサイズ
        self.num_prior = len(cfg['feature_maps'])   # sourcreの個数=6
        self.steps = cfg['steps']   # [8, 16, ...] DBoxのピクセルサイズ
        self.min_sizes = cfg['min_sizes']   # [30, 40, ...]　小さい正方形のDBoxのピクセルサイズ
    ...
    def make_dbox_list(self):
        ...
        for k, f in enumerate(self.feature_maps):
            for i, j in product(range(f), repeat = 2):
                ...
                # 'max_sizes' = [60, 111, 162, 2134, 264, 315]
...
class SSD(nn.Module):
    def __init__(self, phase, cfg):
        ...
        self.L2Norm = L2Norm()
        self.loc, self.cong = make_loc_conf(
    ...
    def forward(self, x):
        ...
        # sources1~6にそれぞれ対応する畳み込みを1回ずつ適用する
        # zipでforループの複数のリストの要素を取得
        # sourcee1~6まであるので、6回ループが回る
        for (x, l , c) in zip(sources, self.loc, self.conf):
            # Permuteは要素の順番を入れ替える
            loc.append(l(x).permute(0, 2, 3, 1).contigunous())
            conf.append(c(x).permnute(0, 2, 3, 1).contigunous())
...
def nm_suppression(boxes, scores, overlap=0.45, top_k=200):
    ...
    # keep: torch.Size([各進度閾値を超えたBBox数])、要素は全部0
    ...
    # boxesと同じtorch.dtypeとtorch.deviceを持つテンソルを作成する。
    # 後で、BBoxのかぶり度合いIOUの計算に使用する際のひな形として用意
    tmp_x1 = boxes.new()
    tmp_y1 = boxes.new()
    tmp_x2 = boxes.new()
    tmp_y2 = boxes.new()
    ...
    while idx.numel() > 0:
        ...
        # keepの現在の最後にconfが最大のindexを格納する
        # このindexのBBoxとかぶりが大きいBBoxをこれから消去する
        keep [count] = i
        count += 1
        ...
        # すべてのBboxに対して、現在のBBox=indexがiとかぶっている値までにclamp
...
class Detect(Function):
    ...
    def forward(self, loc_data, conf_data, dbox_list):
        ...
        Args:
            loc_data (torch.Tensor): オフセット情報
            conf_data (torch.Tensor): 検出の確信度
            dbox_list (torch.Tensor): DBoxの情報

        Returns:
            torch.Tensor: torch.Size([batch_num, num_classes, top_k, 5])
        """
        # 各サイズを取得
        ...
        # 出力の型を作成する。テンソルサイズは[minibatch_num, num_classes, top_k, 5]
        output = torch.zeros(num_batch, num_classes, self.top_k, 5)

        # conf_dataを[batch_num, 8732, num_classes]から[batch_nmum, num_classes, 8732]に順番変更
        ...
        for i in range(num_batch):
            ...
            for cl in range(1, num_classes):
                ...
                # gtは Greater Than のこと。gtにより閾値を超えたものが1に、それ以下が0になる。
                ...
                # c_maskを、decode_boxesに適用できるようにサイズを変更
                l_mask = c_mask.unsqueeze(1).expand_as(decoded_boxes)
                # l_mask: torch.Size([8732, 4])

                # l_maskをdecoded_boxesに適応させる
                boxes = decoded_boxes[l_mask].view(-1, 4)
                # decoded_boxes[l_mask]で1次元になってしまうので、
                # view で（閾値を超えたBBox数, 4）サイズに変形しなおす

                # 3. Non-Maximum Suppressinを実施し、被っているBBoxを取り除く
                ids, count = nm_suppression(
                    boxes, scores, self.nms_thresh, self.top_k
                )
                # ids: confの降順にNon-Maximum Suppression を通過したindexが格納
